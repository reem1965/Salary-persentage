...
st.title("حاسبة المستحقات الشهرية المتراكمة (بالشيكل)")

# البيانات المستخرجة من العمود الأخير فقط
months = [
    "Nov-21", "Dec-21",
...
# حساب الفروقات الشهرية
df["النسبة_الشهرية"] = df["النسبة_التراكمية"].diff().fillna(df["النسبة_التراكمية"])

# إدخال الراتب الشهري
salary = st.number_input("أدخل الراتب الشهري (شيكل):", min_value=0.0, step=100.0)

# حساب المستحقات الشهرية
df["المستحق_هذا_الشهر"] = df["النسبة_الشهرية"] / 100 * salary

# حساب الإجمالي
total_due = df["المستحق_هذا_الشهر"].sum()

# عرض النتائج
st.subheader("تفاصيل المستحقات الشهرية:")
st.dataframe(df[["الشهر", "النسبة_الشهرية", "المستحق_هذا_الشهر"]])

st.subheader(f"المجموع الكلي لجميع المستحقات: {total_due:,.2f} شيكل")